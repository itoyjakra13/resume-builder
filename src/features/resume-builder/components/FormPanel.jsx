...
export const FormPanel = memo(function FormPanel({ errors = {}, setErrors }) {
  ...
  const getTabName = (id) => {
    const found = FORM_TABS.find(t => t.id === id);
      ...
      >
        {FORM_TABS.map((tab) => (
          <button
            ...
            className={`px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all border-b-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 cursor-pointer ${
              activeTab === tab.id
